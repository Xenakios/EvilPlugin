...
template <typename Cont, typename F> inline void erase(Cont &c, F f)
{
...
struct ValueBetween
{
    ...
    bool operator()(EnvelopePoint &pt) const { return pt.getY() >= m_a && pt.getY() < m_b; }
};
...
class Envelope
{
  public:
    Envelope() {}
    Envelope(std::initializer_list<EnvelopePoint> points) : m_points(points) { sortPoints(); }
    ...
    {
        // erase(m_points, [t0, t1](const EnvelopePoint& a) { return a.getX() >= t0 && a.getX() <=
        // t1; });
    }
    template <typename F> void removePointsConditionally(F f) { erase(m_points, f); }
    void sortPoints()
    {
        std::stable_sort(
    ...
    // Not super efficient, if performance is really wanted, should implement a separate envelope
    // playback object that internally keeps track of the current point index etc...
    double getValueAtTime(double time) const
    {
        ...
        auto it = std::lower_bound(
            m_points.begin(), m_points.end(), to_search,
            [](const EnvelopePoint &a, const EnvelopePoint &b) { return a.getX() < b.getX(); });
        if (it == m_points.end())
        {
            return m_points.back().getY();
    ...
    void applyToBuffer(double *buf, int bufsize, double t0, double t1,
                       Range<double> limitrange = {}) const
    {
        int curnode = -1;
...
static std::pair<int64_t, double> CPU_waster(std::mt19937 &rng, double durationtowaste)
{
